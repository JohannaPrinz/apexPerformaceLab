...
import { readTrendCards, trendCardsPayload, withoutTrendCard, withTrendCard } from '@apex/domain';
...
/** The cards this athlete's profile shows. Empty until the coach chooses. */
export async function trendCardsFor(
  db: TrackingDb,
  tenant: Pick<TenantContext, 'organizationId'>,
  athleteId: string,
  ...
  return athlete === null ? [] : readTrendCards(athlete.trendCards);
}
...
export async function setTrendCard(
  ...
  const { count } = await db.athlete.updateMany({
    where: scoped(tenant, { id: athleteId }),
    data: { trendCards: trendCardsPayload(next) },
  });
